...
class QuadTreeNodeEmpty(QuadTreeNode):
    ...
    def mirror_copy(self) -> QuadTreeNodeEmpty:
...
class QuadTreeNodeInternal(QuadTreeNode):
    ...
    def convert_to_pixels(self, width: int, height: int) -> List[List[int]]:
        ...
        result = []
        bottom_left = self.children[0].convert_to_pixels(width_half,
                                                         height_half)
        bottom_right = self.children[1].convert_to_pixels(width - width_half,
                                                          height_half)

        for i in range(height_half):
            result.append(bottom_left[i] + bottom_right[i])

        top_left = self.children[2].convert_to_pixels(width_half,
                                                      height - height_half)
        top_right = self.children[3].convert_to_pixels(width - width_half,
                                                       height - height_half)

        for i in range(height - height_half):
    ...
    def restore_from_preorder(self, lst: List[str], start: int) -> int:
        ...
        for i in range(4):
            ...
            if item == 'E':
                ...
                counter += self.children[i].restore_from_preorder(lst, start +
                                                                  counter)
        return counter
    ...
    def mirror(self) -> None:
        ...
        """
        self.children[2] = self.children[0].mirror_copy()
        self.children[3] = self.children[1].mirror_copy()

    def mirror_copy(self) -> QuadTreeNodeInternal:
        node = QuadTreeNodeInternal()

        node.children = [self.children[2].mirror_copy(),
                         self.children[3].mirror_copy(),
                         self.children[0].mirror_copy(),
                         self.children[1].mirror_copy()]
        return node
...
class QuadTree:
    ...
    def _build_tree_helper(self, pixels: List[List[int]]) -> QuadTreeNode:
        ...
        standard_dev, mean = standard_deviation_and_mean(pixels)

        if standard_dev <= self.loss_level:
            mean_color, _ = mean_and_count(pixels)
            return QuadTreeNodeLeaf(round(mean_color))

        parent = QuadTreeNodeInternal()
        parent.pixels = pixels

        for i, quadrant in enumerate(self._split_quadrants(pixels)):
            node = self._build_tree_helper(quadrant)
    ...
    def _split_quadrants(pixels: List[List[int]]) -> List[List[List[int]]]:
        ...
        result.append(QuadTree._sub_matrix(pixels, 0, row_mid, 0, col_mid))
        result.append(QuadTree._sub_matrix(pixels, 0, row_mid, col_mid, y))
        result.append(QuadTree._sub_matrix(pixels, row_mid, x, 0, col_mid))
        result.append(QuadTree._sub_matrix(pixels, row_mid, x, col_mid, y))
        return result

    @staticmethod
    def _sub_matrix(pixels: List[List[int]], row_start, row_end, col_start,
                    col_end) -> List[List[int]]:
        result = []
        for i in range(row_start, row_end):
            result.append([])
            for j in range(col_start, col_end):
                result[-1].append(pixels[i][j])
...
def maximum_loss(original: QuadTreeNode, compressed: QuadTreeNode) -> float:
    ...
    1.5811388300841898
    """
    if isinstance(original, QuadTreeNodeInternal) and not isinstance(compressed, QuadTreeNodeInternal):
        standard_dev, mean = standard_deviation_and_mean(original.pixels)
        return standard_dev

    elif not isinstance(original, QuadTreeNodeInternal):
        return 0

    return max(
        maximum_loss(original.children[i], compressed.children[i]) for i in
        range(4))
