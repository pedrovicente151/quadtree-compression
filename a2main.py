...
if __name__ == '__main__':
    ...
    print("====================================")
    print("Welcome to the Quad Tree Compressor!")
    print("====================================")
    print("===== Please select a command. =====")

    while True:
        command = input("[c -> Compress | d -> Decompress | q -> quit] : ")

        if command == 'q':
            break

        if command == 'c':
            loss = int(input('Loss [between 0 - 255]: '))
            filename = input("File: ")
            compressor = Compressor(filename, loss)
            compressor.run()
